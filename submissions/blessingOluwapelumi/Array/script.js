...
function productArray() {
	for (a = 0; a < productBrands.length; a++) {
		if (productBrands[a].length % 2 === 0) {
			newProductPrice = productCurrentPrice[a] * 2;
			document.write(`The price of ${productBrands[a]} is ${newProductPrice} <br>`);
		} else {
			newProductPrice = productCurrentPrice[a] / 2;
			document.write(
				`The price of ${"  " + productBrands[a]} is ${"  " + newProductPrice} <br>`
			);
		}
	}
}
