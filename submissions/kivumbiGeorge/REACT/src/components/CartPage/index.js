...
class CartPage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            total: 0, deliveryPrice: 1200, items: [{
                image: KFCKingBucketSVG, product: "KFC - King Bucket 1", number: 2, unitPrice: 5000, totalPrice: 10000,
            }, {
                image: RefuelMaxPotSVG, product: "Refuel Max", number: 2, unitPrice: 600, totalPrice: 1200,
            }, {
                image: KFCKingBucket2SVG, product: "KFC - King Bucket 2", number: 2, unitPrice: 600, totalPrice: 1200,
            },],
        };
    ...
    handleIncrease(idx) {
        ...
        this.setState((state) => ({items: this.state.items.map((item, index) => index === idx ? oldItem : item)}));
        /* update total price */
        this.setState((state) => ({total: this.calculateTotal(this.state.items)}));
    }
    ...
    handleReduce(idx) {
        ...
        if (newNumber < 1) {
            ...
            this.setState((state) => ({items: this.state.items.filter((item, index) => index !== idx)}))
        } else {
                ...
                <div className="cart-body">
                    <div className="cart-body-header-section">
                        <div className="cart-body-title-section">
                            ...
                            <p className="cart-body-item-number"><span>{this.state.items.length}</span>&nbsp;items</p>
                        </div>
                        {this.state.items.length > 0 ? <div className="cart-body-remove-section">
                            <a href="#" className="cart-body-remove-link" onClick={this.handleDelete}>Remove all</a>
                        </div> : <></>}
                    </div>
                    <div className="cart-list">
                        {/*{this.state.items.length > 0 ? this.state.items.map((item, index) =>*/}
                        {/*        <CartItem image={item.image} product={item.product} number={item.number}*/}
                        {/*                  price={item.totalPrice} handleIncrease={this.handleIncrease} key={index}*/}
                        {/*                  handleReduce={this.handleReduce} handleRemove={this.handleRemove} index={index}/>)*/}
                        {/*    :*/}
                        {/*    <div className="cart-list-empty">*/}
                        {/*        <h3 className="cart-list-empty-title">*/}
                        {/*            No Items In Cart!*/}
                        {/*        </h3>*/}
                        {/*    </div>*/}
                        {/*}*/}
                        {/*<CartItem image={ChickenKFCSVG} product="KFC - King Bucket" number={2} price={1000}/>*/}
                        {/*<CartItem image={ChickenKFCSVG} product="KFC - King Bucket" number={2} price={1000}/>*/}
                        {/*<CartItem image={ChickenKFCSVG} product="KFC - King Bucket" number={2} price={1000}/>*/}
                        {this.state.items.length > 0 ? this.state.items.map((item, index) =>
                            ...
                            <div className="cart-item-detail">
                                <div className="delete-item">
                                    <button className="delete-item-button">
                                        <img alt="close" className="delete-item-image" src={CloseSVG}
                                             onClick={() => this.handleRemove(index)}/>
                                    </button>
                                </div>
                                <div className="cart-item-product">
                                    <div className="cart-item-product-image-container">
                                        <img alt="FImg" className="cart-item-product-image" src={item.image}/>
                                    </div>
                                    <div className="cart-item-product-description">
                                        ...
                                        <div className="cart-item-product-amount">
                                            ...
                                            <div className="cart-item-number">
                                                <div className="cart-item-number-value">{item.number}</div>
                                            </div>
                    ...
                    <div className="cart-delivery">
                        <div className="cart-delivery-detail">
                            <div className="cart-delivery-description">
                                <img alt="dl" className="cart-delivery-description-image" src={DeliverySVG}/>
                                <h3 className="cart-delivery-description-title">Delivery Fee</h3>
                            </div>
                            <div className="cart-delivery-price">
                                <p className="cart-delivery-price-value">#{this.state.items.length > 0 ? this.state.deliveryPrice : 0}</p>
                            </div>
                        </div>
                        <div className="cart-delivery-border">
                        </div>
                    </div>
