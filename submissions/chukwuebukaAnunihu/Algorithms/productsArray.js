...
function printProductPrice(products, product_price) {
  let newProductPrice = [];
  let output = ""
  for (let i = 0; i < products.length; i++) {
    if (products[i].length % 2 == 0) {
      for (let j = 0; j < product_price.length; j++) {
        if (i == j) {
          newProductPrice.push(product_price[j] * 2);
        } else {
          newProductPrice.push(product_price[j] / 2);
        }
      }
    }
    output = `<p> The price of ${products[i]} is $${newProductPrice[i].toFixed(2)} </p>`
    document.write(output);
  }
  return output;
}

printProductPrice(products, product_price);
