...
check_file_content2() {    
    for currentFile in $path_to_php_folder/*
    do
        if grep  "101"  $currentFile && grep "150" $currentFile
        then
            content_exists=1
            break     
        else
            ...
            echo -e "1. Ensure the program displays every number between 101 and 150"${NONE}
        fi
   done
}
...
check_file_content3() {    
    for currentFile in $path_to_php_folder/*
    do
        if grep  "<"  $currentFile && grep "50" $currentFile
        then
            content_exists=1
            break     
        else
            content_exists=0
            echo -e "${BOLD}Checking the php file ...${NONE}"
            echo -e "1. Ensure the program diplays all even numbers between 0 and 50"${NONE}
        fi
   done
}
...
check_file_content4() {    
    for currentFile in $path_to_php_folder/*
    do
        if grep "%"  $currentFile && grep "2" $currentFile
        then
            content_exists=1
            break     
        else
            content_exists=0
            echo -e "${BOLD}Checking the php file ...${NONE}"
            echo -e "1. Ensure the program display even numbers"${NONE}
        fi
   done
}
...
check_file_content5() {    
    for currentFile in $path_to_php_folder/*
    do
        if grep "<br>"  $currentFile && grep "=" $currentFile
        then
            content_exists=1
            break     
        else
            content_exists=0
            echo -e "${BOLD}Checking the php file ...${NONE}"
            echo -e "1. Ensure each result displayed is on a new line"${NONE}
        fi
   done
}
