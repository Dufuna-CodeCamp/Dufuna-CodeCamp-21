...
path_to_programmingLanguages_folder="$path_to_folder/programmingLanguages"
...
check_programmingLanguages_folder_existence() {
    folder_count=$(ls $path_to_folder | grep "programmingLanguages" -c)
    if [ $folder_count -eq 0 ]
    then
...
check_file_existence() {
    if [ -z "$(ls -A $path_to_programmingLanguages_folder)" ]
    then
        file_exists=0
        echo ""
        echo -e "${BOLD}Checking the text file ...${NONE}"
        echo -e "${RED}1. You should create a text file."
        echo -e "2. Ensure your text file is located in the ${BOLD}\"programmingLanguages\"${NONE} ${RED}folder you have created."
