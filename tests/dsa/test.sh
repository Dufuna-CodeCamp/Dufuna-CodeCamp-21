...
path_to_dsa_folder="$path_to_folder/DSA"
...
check_dsa_folder_existence() {
    folder_count=$(ls $path_to_folder | grep "DSA" -c)
    if [ $folder_count -eq 0 ]
    then
        dsa_folder_exists=0
        echo ""
        echo -e "${BOLD}Checking the dsa folder ...${NONE}"
        echo -e "${RED}Your folder should be named ${BOLD}\"DSA\"${NONE} ${RED}and located in the folder with your First and Last name. (${BOLD}\"$folder_name/dsa\"${NONE}${RED})${NONE}"
    else
        dsa_folder_exists=1
    fi
}
...
check_file_existence() {
    if [ -z "$(ls -A $path_to_dsa_folder)" ]
    then
        file_exists=0
        echo ""
        echo -e "${BOLD}Checking the text file ...${NONE}"
        echo -e "${RED}1. You should create a text file."
        echo -e "2. Ensure your text file is located in the ${BOLD}\"DSA\"${NONE} ${RED}folder you have created."
        echo -e "3. Ensure your text file contains text."${NONE}
    else
        for currentFile in $path_to_dsa_folder/*
        do
