...
check_file_content1() {    
    for currentFile in $path_to_database_folder/*
    do
        if grep  "CREATE TABLE admins"  $currentFile  && grep "CREATE TABLE vendor_food" $currentFile
        then
            content_exists=1
            break     
        else
...
check_file_content2() {    
    for currentFile in $path_to_database_folder/*
    do
        if grep  "CREATE TABLE orders"  $currentFile && grep "CREATE TABLE food" $currentFile
        then
            content_exists=1
            break     
        else
...
check_file_content3() {    
    for currentFile in $path_to_database_folder/*
    do
        if grep  "id INT NOT NULL AUTO_INCREMENT"  $currentFile && grep "name VARCHAR(100) NOT NULL" $currentFile
        then
            content_exists=1
            break     
        else
...
check_file_content4() {    
    for currentFile in $path_to_database_folder/*
    do
        if grep "admin_id INT NOT NULL"  $currentFile && grep "password VARCHAR(100) NOT NULL" $currentFile
        then
            content_exists=1
            break     
        else
